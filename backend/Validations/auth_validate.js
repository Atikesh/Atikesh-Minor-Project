...
const loginSchema = z.object({
    ...
    password: z
    .string({required_error:"Password is required"}).trim().min(8, {message: "Name must be at least of 8 character's"}),
});
...
const signupSchema = z.object({
    ...
    .string({required_error:"Email is required"}).trim().min(5, {message: "Name must be at least of 5 character's"}).max(255, {message: "Name must not be more than 255 character's"}).email({message:"Invalid email address"}),
    contact: z
    .string({required_error:"Contact No. is required"}).trim().min(10, {message: "Enter must be at least of 10 number's"}),
    password: z
    .string({required_error:"Password is required"}).trim().min(8, {message: "Name must be at least of 8 character's"}),
});
